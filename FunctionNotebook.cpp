...
FunctionNotebook::FunctionNotebook(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name)
...
void FunctionNotebook::LoadPageData(int page, InformData *data)
{
  if(data!=NULL) function=(FunctionData *) data;
  ...
  case 0: // Source code
    ...
    sourceEditor->SetValue(function->GetSource(INF_SOURCE));
    break;
  }
}

bool FunctionNotebook::SavePageData(int page, wxTreeEvent *event)
{
  ...
  case 0: // Source code
    ...
    if(sourceEditor->GetModified()) {
      function->SetSource(INF_SOURCE, sourceEditor->GetValue());
    }
    break;
  }

  if(frame->project.UpdateAll(event, function)) LoadPageData();
  return false;
}
